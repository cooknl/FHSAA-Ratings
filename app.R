    ...
    sidebarLayout(
        sidebarPanel(
            ...
            checkboxGroupInput(
                inputId = "divs_check",
                label = "Divisions:",
                choices = unique(divisions$Division),
                selected = unique(divisions$Division)
            )
        ),
        
        # Show a plot of the generated distribution
        mainPanel(plotlyOutput("FLmap"))
    ),
    actionButton(
        inputId ="check_rankings",
        label = "Check for Updated Rankings"
    ),
    actionButton(
        inputId ="update_rankings",
...
server <- function(input, output,session) {
    ...
    observeEvent(input$check_rankings, {
        ...
        if ((old_rankingsDate  %--%  rankingsDate) %/% days(1)) {
            vals$date_check = paste("A rankings update is available as of", day(rankingsDate), month(rankingsDate, label=TRUE), year(rankingsDate), sep = " ")
        }
        else
    ...
    observeEvent(input$update_rankings, {
        ...
        source("./get-fhsaa-rankings.R")
        load("./divisions.RData")
        vals$date_check = paste("Rankings have been updated and are", (rankingsDate %--% as_datetime(today())) %/% days(1), "days old.", sep = " ")
    })
    ...
    selected <- reactive({
        (divisions$Rating > input$ratings[[1]]) &
            ...
            (divisions$Division %in% input$divs_check)
    })
    ...
    output$FLmap <- renderPlotly({
        ...
        ggplotly(lax_map) %>% 
            ...
            plotly::config(modeBarButtonsToRemove = c("select2d",
                                                      "lasso2d",
                                                      "autoScale2d",
    ...
    output$dateInfo <- renderText({
        if (is.null(vals$date_check))
            paste("Rankings are", (rankingsDate %--% as_datetime(today())) %/% days(1), "days old.", sep = " ")
        else
            vals$date_check
    })
